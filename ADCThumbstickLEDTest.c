...
uint32_t Adirection[2];  
  ...
  while((ADC1_RIS_R&0x04)==0){};   // 2) wait for conversion done ADC raw interrupt status, 
		...
		if (Adirection[1] <= 2196 && Adirection[1] >= 1900) //the x direction center 
			GPIO_PORTF_DATA_R = 0x00;
		else if (Adirection[1] <= 2100)
			GPIO_PORTF_DATA_R = 0x08;
		else 
			GPIO_PORTF_DATA_R = 0x02;
		if (Adirection[0] <= 2196 && Adirection[0] >= 1900) //the y direction center 
			GPIO_PORTF_DATA_R = 0x00;
		else if (Adirection[0] <= 2100)
